...
### 1.3 Namespace & Module Resolution
...
Functions requiring internal state use the **lambda block** (`|>`) syntax:
...
- Trigger parameter: `name!` (postfixed `!` indicates trigger input)
...
The `emit` keyword marks variables for persistence across update cycles.
...
**Behavior**:
- Variables with `emit` retain their value across updates
- Without `emit`, variables are local to the update (computed fresh each cycle)
- Makes state mutation explicit and auditable
...
```idl
// Sine table
sine_table(size) = [for i in 0..size : sin(i / size * 2 * pi)]

// Arithmetic sequence
intervals = [for i in 0..8 : i * 2]

// Harmonic series
harmonics(fundamental) = [for h in 1..16 : fundamental * h]
```
...
Convert bitwise operations to trigger flows:
...
**Idyl Libraries** (`.idl` files):
```idl
lib("scales.idl")
lib("synth_utils.idl")
```

**External Modules** (compiled binaries, C libraries, etc.):
```idl
cs = import("csound")
osc = import("osc")
midi = import("midi")
...
**Important**: Import and library statements are **global scope only**. They cannot appear inside process blocks.
...
trait melody_operations {
    op+(a : flow, b : flow) = [for i : max(a[i], b[i])]
    op*(sequence : flow, factor : number) = [for i : sequence[i] * factor]
}
...
filter_range = [for i in 0..100 : i where (i % 2 == 0)]
```
